...
def show_weights_input(user_input, node_data, manual_class):
    manual_active = False
    if manual_class == 'auto-manual-button active':
        manual_active = True

    if node_data and user_input is None and manual_active is False:
        return (neither_node_nor_input, 'weights-input', 'hidden-opacity',
                'visible-opacity',
                'hidden-opacity', 'hidden-opacity',)
    elif node_data and user_input is None and manual_active is True:
        return (neither_node_nor_input, 'weights-input', 'hidden-opacity',
                'visible-opacity',
                'visible-opacity', 'visible-opacity',)
    elif node_data and (user_input is not None or '') and manual_active is False:
        return (only_node, 'weights-input', 'weights-input-button',
                'visible-opacity',
                'hidden-opacity', 'hidden-opacity')
    elif node_data and (user_input is not None or '') and manual_active is True:
        return (only_node, 'weights-input', 'weights-input-button',
                'visible-opacity',
                'visible-opacity', 'visible-opacity')
    else:
