...
@app.callback(
    [
        ...
        Output('custom-update-button', 'n_clicks')
    ],
    [
...
def fields_update(update_clicks, tap_node,
        ...
        for element in elements:
            if 'source' not in element['data'] and element['data'].get('id') == selected_node_id:
                ...
                if custom2_input not in (None, ''):
                    ...
                    custom1_input = stored_names[1]
                if custom3_input not in (None, ''):
                    element['data']['custom3']['field_name'] = custom3_input
                else:
                    custom1_input = stored_names[2]
                value1, value2, value3 = get_node_values(elements, selected_node_id)
                names_list = [custom1_input, custom2_input, custom3_input]
                return (elements, '', '', '', custom1_input, custom2_input, custom3_input, custom1_input, custom2_input,
                        custom3_input, value1, value2, value3, names_list, 0)
    ...
    value1, value2, value3 = get_node_values(elements, selected_node_id)
    return elements, '', '', '', name1, name2, name3, name1, name2, name3, value1, value2, value3, stored_names, 0
